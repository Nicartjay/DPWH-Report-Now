...
| Absolute endpoint URLs in frontend | Relative URLs (`/api/chat`) |

## Files Added

| File | Purpose |
|------|---------|
| `pages/api/chat.js` | AI proxy API route (replaces `server.js`) |
...
| `public/src/js/services/aiChatService.js` | Endpoint changed from `http://localhost:3000/api/chat` to `/api/chat` |
...
- **Production-ready** — Easy deployment to Vercel, Netlify, etc.
